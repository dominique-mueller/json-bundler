...
describe('JSON bundler', () => {
  ...
  describe('(one-level references)', () => {
    const paths: Array<string> = ['./shared/copyright'];
  ...
  describe('(two-level references)', () => {
    const paths: Array<string> = ['./shared/footer', './shared/copyright'];
  ...
  describe('(node_modules references)', () => {
    const externalPaths: Array<string> = ['./json-bundler-test/copyright'];
  ...
  describe('(errors)', () => {
    it('should throw an error if a referenced file is not JSON / JSON5', () => {
      ...
      expect(error.message.split('\n')[0]).toBe(
        `The file "${path.resolve('./test/errors/src/does-not-exist.txt')}" is not of type JSON / JSON5.`,
      );
      expect(error.message.split('\n')[1]).toBe(
        `Details: File is referenced in "${path.resolve('./test/errors/src/input-invalid-file-type.json')}".`,
      );
    });
    ...
    it('should throw an error if a referenced file does not exist', () => {
      ...
      expect(error).not.toBeNull();
      expect(error.message.split('\n')[0]).toContain(
        `An error occured while reading the file "${path.resolve('./test/errors/src/does-not-exist.json')}".`,
      );
      expect(error.message.split('\n')[1]).toBe(
        `Details: File is referenced in "${path.resolve('./test/errors/src/input-reference-does-not-exist.json')}".`,
      );
    });
    ...
    it('should throw an error if a referenced file is invalid JSON', () => {
      ...
      expect(error).not.toBeNull();
      expect(error.message.split('\n')[0]).toContain(
        `An error occured while parsing the file "${path.resolve('./test/errors/src/shared/invalid.json')}" as JSON.`,
      );
      expect(error.message.split('\n')[1]).toBe(
        `Details: File is referenced in "${path.resolve('./test/errors/src/input-invalid-syntax.json')}".`,
      );
    });
    ...
    it('should throw an error if a referenced file is invalid JSON5', () => {
      ...
      expect(error).not.toBeNull();
      expect(error.message.split('\n')[0]).toContain(
        `An error occured while parsing the file "${path.resolve('./test/errors/src/shared/invalid.json5')}" as JSON5.`,
      );
      expect(error.message.split('\n')[1]).toBe(
        `Details: File is referenced in "${path.resolve('./test/errors/src/input-invalid-syntax.json5')}".`,
      );
    });
...
function test(folderName: string, jsonType: 'json' | 'json5', minified: boolean, paths: Array<string>, externalPaths: Array<string>): void {
  ...
  const expectedPaths: Array<string> = ['input', ...paths].map((filePath: string): string => {
    return path.resolve('test', folderName, 'src', `${filePath}.${jsonType}`);
  });
  const expectedExternalPaths: Array<string> = externalPaths.map((externalFilePath: string): string => {
